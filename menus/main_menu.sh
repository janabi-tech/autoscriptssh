...
execute_add_user() {
    while true; do
        ...
        /opt/janabitech/bin/janabitech user add "$USERNAME" "$PASSWORD" "$DAYS" "$MAX_LOGINS" "$BW_LIMIT" > /dev/null 2>&1
        API_STATUS=$?
...
execute_trial_user() {
    ...
    /opt/janabitech/bin/janabitech user trial "$USERNAME" "$PASSWORD" "$HOURS" "$MAX_LOGINS" "$BW_LIMIT" > /dev/null 2>&1
...
menu_services() {
    while true; do
        ...
        echo -e "  ${CYAN}Xray vmess (ws)   :${NC} 8080 / 2053"
        echo -e "  ${CYAN}Xray vmess (grpc) :${NC} 2087"
        echo -e "  ${CYAN}Xray vless (ws)   :${NC} 2052 / 2083"
        echo -e "  ${CYAN}Xray vless (grpc) :${NC} 8443"
        echo -e "  ${CYAN}Xray trojan (ws)  :${NC} 2082 / 2096"
        echo -e "  ${CYAN}Xray trojan (grpc):${NC} 10008"
        draw_line
        ...
        echo -e "  ${CYAN}[06]${NC} Restart Xray-core"
        echo -e ""
        echo -e "  ${RED}[00]${NC} Return to Main Menu"
        draw_line
        read -p " Select Option : " opt
        ...
        case $opt in
            ...
            6) /opt/janabitech/bin/janabitech xray restart; pause ;;
            0) return ;;
            *) echo -e "${RED}Invalid option${NC}"; sleep 1 ;;
        esac
    done
        ...
        case $opt in
            1) 
                ...
                case $rb_opt in
                    ...
                    0) /opt/janabitech/bin/janabitech sys autoreboot 0 ;;
...
# ==========================================================
# [07] XRAY MANAGER  (vmess / vless / trojan over ws-nontls / ws-tls / grpc)
# ==========================================================
menu_xray_manager() {
    while true; do
        clear
        ...
        local xray_state="OFF"
        if systemctl is-active --quiet janabitech-xray 2>/dev/null; then
            xray_state="${GREEN}[ON]${NC}"
        else
            xray_state="${RED}[OFF]${NC}"
        fi

        local xray_total xray_active
        xray_total=$(sqlite3 "$DB_PATH" "SELECT COUNT(*) FROM xray_users;" 2>/dev/null || echo 0)
        xray_active=$(sqlite3 "$DB_PATH" "SELECT COUNT(*) FROM xray_users WHERE status='ACTIVE';" 2>/dev/null || echo 0)

        draw_line
        echo -e "                  ${BOLD}XRAY MANAGER${NC}                      "
        draw_line
        echo -e "  ${ORANGE}Host${NC}        : ${GREEN}${PRIMARY_DOMAIN}${NC}"
        echo -e "  ${ORANGE}Cert${NC}        : /opt/janabitech/core/keys/fullchain.cer"
        echo -e "  ${ORANGE}Service${NC}     : ${xray_state}   ${ORANGE}Accounts${NC}: ${GREEN}${xray_active}${NC}/${xray_total}"
        echo -e "  ${ORANGE}Protocols${NC}   : vmess / vless / trojan"
        echo -e "  ${ORANGE}Transports${NC}  : ws (no TLS) | ws (TLS) | gRPC (TLS)"
        draw_line
        echo -e "  ${CYAN}[01]${NC} Create Xray Account (vmess)"
        echo -e "  ${CYAN}[02]${NC} Create Xray Account (vless)"
        echo -e "  ${CYAN}[03]${NC} Create Xray Account (trojan)"
        echo -e "  ${CYAN}[04]${NC} Create Trial (vmess)"
        echo -e "  ${CYAN}[05]${NC} Create Trial (vless)"
        echo -e "  ${CYAN}[06]${NC} Create Trial (trojan)"
        echo -e "  ${CYAN}[07]${NC} Renew Xray Account"
        echo -e "  ${CYAN}[08]${NC} Delete Xray Account"
        echo -e "  ${CYAN}[09]${NC} List Xray Accounts"
        echo -e "  ${CYAN}[10]${NC} Print Account Details (links)"
        echo -e "  ${CYAN}[11]${NC} Restart Xray Service"
        echo -e "  ${CYAN}[12]${NC} Xray Status"
        echo -e "  ${CYAN}[13]${NC} Install / Refresh Xray-core"
        echo -e "  ${CYAN}[14]${NC} Uninstall Xray-core"
        echo -e ""
        echo -e "  ${RED}[00]${NC} Return to Main Menu"
        draw_line
        read -p " Select Option : " opt
        ...
        case $opt in
            1) _xray_create_interactive vmess ;;
            2) _xray_create_interactive vless ;;
            3) _xray_create_interactive trojan ;;
            4) _xray_trial_interactive vmess ;;
            5) _xray_trial_interactive vless ;;
            6) _xray_trial_interactive trojan ;;
            7) _xray_renew_interactive ;;
            8) _xray_delete_interactive ;;
            9) clear; xray_list_users; pause ;;
            10) _xray_details_interactive ;;
            11) clear; /opt/janabitech/bin/janabitech xray restart; pause ;;
            12) clear; /opt/janabitech/bin/janabitech xray status; pause ;;
            13)
                clear
                echo -e "${CYAN}=== INSTALL / REFRESH XRAY-CORE ===${NC}"
                /opt/janabitech/bin/janabitech xray install
                pause ;;
            14)
                clear
                echo -e "${RED}${BOLD}WARNING: This removes Xray-core, all xray accounts,${NC}"
                echo -e "${RED}${BOLD}and the systemd service. SSH/WS/DNSTT are NOT affected.${NC}"
                read -p "Type 'YES' to confirm: " confirm_xray_uninstall
                if [ "$confirm_xray_uninstall" == "YES" ]; then
                    /opt/janabitech/bin/janabitech xray uninstall
                else
...
_xray_create_interactive() {
    local proto="$1"
    clear
    echo -e "${CYAN}=== CREATE XRAY ACCOUNT (${proto^^}) ===${NC}"

    read -p "Username: " USERNAME
    if [[ -z "$USERNAME" ]]; then
        echo -e "${RED}[ERROR] Username cannot be empty.${NC}"; sleep 1.5; return
    fi

    read -p "Duration value (e.g., 30): " DUR_VAL
    DUR_VAL=${DUR_VAL:-30}
    if ! [[ "$DUR_VAL" =~ ^-?[0-9]+$ ]]; then
        echo -e "${RED}[-] Invalid number.${NC}"; sleep 1.5; return
    fi

    echo "Select duration unit:"
    local unit
    unit=$(_xray_pick_unit)

    /opt/janabitech/bin/janabitech xray add "$USERNAME" "$proto" "$DUR_VAL" "$unit" > /dev/null 2>&1
    local rc=$?
    if [ $rc -eq 2 ]; then
        echo -e "\n${ORANGE}[!] User '${USERNAME}' already exists.${NC}"
        pause; return
    elif [ $rc -ne 0 ]; then
        echo -e "\n${RED}[-] Failed to create account. Check logs.${NC}"
        pause; return
    fi

    clear
    echo -e "${GREEN}Xray ${proto^^} account created successfully!${NC}\n"
    /opt/janabitech/bin/janabitech xray details "$USERNAME"
    pause
}

_xray_trial_interactive() {
    local proto="$1"
    clear
    echo -e "${CYAN}=== CREATE XRAY TRIAL (${proto^^}) ===${NC}"

    local USERNAME="trial-${proto}-$((RANDOM % 9000 + 1000))"
    echo -e "Generated Username : ${GREEN}${USERNAME}${NC}"

    read -p "Duration in minutes [default: 30]: " MIN
    MIN=${MIN:-30}
    if ! [[ "$MIN" =~ ^[0-9]+$ ]]; then
        echo -e "${RED}[-] Invalid number.${NC}"; sleep 1.5; return
    fi

    /opt/janabitech/bin/janabitech xray trial "$USERNAME" "$proto" "$MIN" > /dev/null 2>&1
    local rc=$?
    if [ $rc -ne 0 ]; then
        echo -e "\n${RED}[-] Failed to create trial. Check logs.${NC}"
        pause; return
    fi

    clear
    echo -e "${GREEN}Xray ${proto^^} trial created successfully!${NC}\n"
    /opt/janabitech/bin/janabitech xray details "$USERNAME"
    pause
}

_xray_renew_interactive() {
    clear
    echo -e "${CYAN}=== RENEW XRAY ACCOUNT ===${NC}"
    _xray_select_user
    if [[ -z "$FINAL_XRAY_USERNAME" ]]; then return; fi

    echo -e "\nUser: ${GREEN}${FINAL_XRAY_USERNAME}${NC} | Protocol: ${FINAL_XRAY_PROTO} | Current Expiry: ${ORANGE}${FINAL_XRAY_EXP}${NC}"
    echo -e "Enter value (positive to add, negative to deduct):"
    read -p "Value: " VAL
    if ! [[ "$VAL" =~ ^-?[0-9]+$ ]]; then
        echo -e "${RED}[-] Invalid number.${NC}"; sleep 1.5; return
    fi
    ...
    /opt/janabitech/bin/janabitech xray renew "$FINAL_XRAY_USERNAME" "$VAL" "$unit" > /dev/null 2>&1
    if [ $? -ne 0 ]; then
        echo -e "${RED}[-] Renew failed.${NC}"; pause; return
    fi

    local new_exp
    new_exp=$(sqlite3 "$DB_PATH" "SELECT expiry_date FROM xray_users WHERE username='${FINAL_XRAY_USERNAME}';" 2>/dev/null)
    echo -e "\n${GREEN}[+] Renewed.${NC}  New expiry: ${GREEN}${new_exp}${NC}"
    pause
}

_xray_delete_interactive() {
    clear
    echo -e "${CYAN}=== DELETE XRAY ACCOUNT(S) ===${NC}"

    local xray_total
    xray_total=$(sqlite3 "$DB_PATH" "SELECT COUNT(*) FROM xray_users;" 2>/dev/null || echo 0)
    if [ "$xray_total" -eq 0 ]; then
        echo -e "\n${ORANGE}[!] No xray accounts.${NC}"; pause; return
    fi

    mapfile -t ULIST < <(sqlite3 -separator '|' "$DB_PATH" "SELECT username,protocol,expiry_date FROM xray_users ORDER BY id ASC;" 2>/dev/null)
    draw_line
    printf "${BOLD}%-5s | %-20s | %-8s | %-20s${NC}\n" "S/N" "USERNAME" "PROTO" "EXPIRY"
    draw_line
    local i=1
    for row in "${ULIST[@]}"; do
        local u=$(echo "$row" | cut -d'|' -f1)
        local p=$(echo "$row" | cut -d'|' -f2)
        local e=$(echo "$row" | cut -d'|' -f3)
        printf "${GREEN}%-5s${NC} | ${CYAN}%-20s${NC} | ${MAGENTA}%-8s${NC} | ${ORANGE}%-20s${NC}\n" "$i" "$u" "$p" "$e"
        ((i++))
    done
    draw_line
    echo -e "Enter S/N(s) separated by commas or ranges (e.g., 1,3 or 2-4)"
    read -p "Select targets: " TARGET_INPUT
    if [[ -z "$TARGET_INPUT" ]]; then return; fi

    local parsed_list
    parsed_list=$(echo "$TARGET_INPUT" | awk -F, '{
        for(i=1; i<=NF; i++) {
            if ($i ~ /-/) { split($i, a, "-"); for(j=a[1]; j<=a[2]; j++) printf "%s ", j }
            else { printf "%s ", $i }
        }
    ...
    local deleted=0
    for sn in $parsed_list; do
        if [[ "$sn" =~ ^[0-9]+$ ]] && [ "$sn" -le "$count" ] && [ "$sn" -gt 0 ]; then
            local idx=$((sn - 1))
            local target=$(echo "${ULIST[$idx]}" | cut -d'|' -f1)
            /opt/janabitech/bin/janabitech xray del "$target" > /dev/null 2>&1
            echo -e "  - Deleted: ${RED}${target}${NC}"
            ((deleted++))
        fi
    done
    [ "$deleted" -eq 0 ] && echo -e "${RED}[-] No valid selection.${NC}"
    pause
}

_xray_details_interactive() {
    clear
    echo -e "${CYAN}=== XRAY ACCOUNT DETAILS ===${NC}"
    _xray_select_user
    if [[ -z "$FINAL_XRAY_USERNAME" ]]; then return; fi
    echo ""
    /opt/janabitech/bin/janabitech xray details "$FINAL_XRAY_USERNAME"
    pause
}
...
_xray_select_user() {
    ...
    xray_total=$(sqlite3 "$DB_PATH" "SELECT COUNT(*) FROM xray_users;" 2>/dev/null || echo 0)
    if [ "$xray_total" -eq 0 ]; then
        echo -e "\n${ORANGE}[!] No xray accounts.${NC}"; sleep 2; FINAL_XRAY_USERNAME=""; return
    fi
    mapfile -t ULIST < <(sqlite3 -separator '|' "$DB_PATH" "SELECT username,protocol,expiry_date FROM xray_users ORDER BY id ASC;" 2>/dev/null)
    draw_line
    printf "${BOLD}%-5s | %-20s | %-8s | %-20s${NC}\n" "S/N" "USERNAME" "PROTO" "EXPIRY"
    ...
    echo -e "${ORANGE}[0] Cancel${NC}\n"
    read -p "Select S/N or type Username: " TARGET_USER
    if [[ "$TARGET_USER" == "0" || -z "$TARGET_USER" ]]; then FINAL_XRAY_USERNAME=""; return; fi

    FINAL_XRAY_USERNAME=""
    if [[ "$TARGET_USER" =~ ^[0-9]+$ ]] && [ "$TARGET_USER" -le "${#ULIST[@]}" ] && [ "$TARGET_USER" -gt 0 ]; then
        local idx=$((TARGET_USER - 1))
        FINAL_XRAY_USERNAME=$(echo "${ULIST[$idx]}" | cut -d'|' -f1)
        FINAL_XRAY_PROTO=$(echo "${ULIST[$idx]}" | cut -d'|' -f2)
        FINAL_XRAY_EXP=$(echo "${ULIST[$idx]}" | cut -d'|' -f3)
    else
        for row in "${ULIST[@]}"; do
            local u=$(echo "$row" | cut -d'|' -f1)
            if [[ "$u" == "$TARGET_USER" ]]; then
                FINAL_XRAY_USERNAME="$u"
                FINAL_XRAY_PROTO=$(echo "$row" | cut -d'|' -f2)
                FINAL_XRAY_EXP=$(echo "$row" | cut -d'|' -f3)
                break
            fi
        done
    fi
    if [[ -z "$FINAL_XRAY_USERNAME" ]]; then
...
show_dashboard() {
    while true; do
        ...
        printf "  ${CYAN}Monitor : %b   Xray    : %b${NC}\n" "$(check_service janabitech-monitor)" "$(check_service janabitech-xray)"
        echo -e ""
        ...
        echo -e "  ${CYAN}[09]${NC} XRAY MANAGER (vmess/vless/trojan)"
        echo -e ""
