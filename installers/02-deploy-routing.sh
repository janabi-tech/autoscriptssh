...
<b><font color="#00ffff">IMAGITECH VPN PREMIUM</font></b><br>
<font color="#00aaaa">════════════════════════════</font><br>
<b><font color="#ff00ff">GET YOUR SUBSCRIPTION</font></b><br>
<font color="#ff00ff">Bot: t.me/imagivpnbot</font><br>
<font color="#ff00ff">Channel: t.me/imagitech001</font><br>
<font color="#00aaaa">════════════════════════════</font><br>
...
<font color="#aaaaaa">imagitech.online</font><br>
<font color="#00aaaa">════════════════════════════</font><br>
</b></p>
EOF
...
if [ -f /opt/imagitech/services/routing/ws-proxy ]; then
    chmod +x /opt/imagitech/services/routing/ws-proxy
else
...
ExecStart=/bin/bash -c 'if [ -f /opt/imagitech/services/routing/ws-proxy ]; then exec /opt/imagitech/services/routing/ws-proxy; else exec /usr/bin/python3 /opt/imagitech/services/routing/ws-proxy.py; fi'
Restart=always
RestartSec=3
LimitNOFILE=1048576
...
if [ -f /opt/imagitech/services/routing/wss-injector ]; then
    chmod +x /opt/imagitech/services/routing/wss-injector
else
    chmod +x /opt/imagitech/services/routing/wss-injector.py 2>/dev/null || true
fi

cat <<EOF > /tmp/imagitech-wss.service.tmp
[Unit]
Description=Imagitech WSS Injection Handler
After=network.target

[Service]
Type=simple
User=root
ExecStart=/bin/bash -c 'if [ -f /opt/imagitech/services/routing/wss-injector ]; then exec /opt/imagitech/services/routing/wss-injector; else exec /usr/bin/python3 /opt/imagitech/services/routing/wss-injector.py; fi'
Restart=always
RestartSec=3
LimitNOFILE=65535
...
safe_deploy_systemd "imagitech-wss"
...
cp /opt/imagitech/core/keys/stunnel.pem /opt/imagitech/core/keys/haproxy.pem
...
frontend https_decrypted
    bind 127.0.0.1:9443 ssl crt /opt/imagitech/core/keys/haproxy.pem accept-proxy
    mode tcp
    tcp-request inspect-delay 5s
    tcp-request content accept if HTTP
...
frontend legacy_ssl
    bind *:8443 ssl crt /opt/imagitech/core/keys/haproxy.pem
    mode tcp
