...
log_event "INFO" "Deploying Core Analytics and Enforcement Engine..."

if [ -f /opt/janabitech/services/monitor/daemon ]; then
    chmod +x /opt/janabitech/services/monitor/daemon
else
...
if [ ! -x "/opt/janabitech/bin/speedtest" ]; then
    run_with_spinner "Installing Analytics Dependencies..." wget -qO /tmp/speedtest.tgz https://install.speedtest.net/app/cli/ookla-speedtest-1.2.0-linux-x86_64.tgz
...
if [ ! -x "/opt/janabitech/bin/btop" ]; then
    run_with_spinner "Installing System Resource Monitor..." wget -qO /tmp/btop.tbz https://github.com/aristocratos/btop/releases/download/v1.3.2/btop-x86_64-linux-musl.tbz
    mkdir -p /tmp/btop
...
ExecStart=/bin/bash -c 'if [ -f /opt/janabitech/services/monitor/daemon ]; then exec /opt/janabitech/services/monitor/daemon; else exec /usr/bin/python3 -u /opt/janabitech/services/monitor/daemon.py; fi'
Restart=always
RestartSec=5
