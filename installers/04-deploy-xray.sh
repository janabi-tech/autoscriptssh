#!/bin/bash
# File: /opt/janabitech/installers/04-deploy-xray.sh
...
safe_create_dir "/opt/janabitech/xray"
...
if [ ! -f "/opt/janabitech/xray/xray" ]; then
    ...
    unzip -q /tmp/xray.zip xray -d /opt/janabitech/xray/
    chmod +x /opt/janabitech/xray/xray
    rm -f /tmp/xray.zip
fi

# 2. Generate REALITY Keys
log_event "INFO" "Generating Next-Gen Cryptographic Keys..."
if [ ! -f "/opt/janabitech/core/keys/reality.pub" ]; then
    XRAY_KEYS=$(/opt/janabitech/xray/xray x25519)
    ...
    echo "$REALITY_PRIV" > /opt/janabitech/core/keys/reality.priv
    echo "$REALITY_PUB" > /opt/janabitech/core/keys/reality.pub
    echo "$REALITY_SHORTID" > /opt/janabitech/core/keys/reality.sid
else
    REALITY_PRIV=$(cat /opt/janabitech/core/keys/reality.priv)
    REALITY_PUB=$(cat /opt/janabitech/core/keys/reality.pub)
    REALITY_SHORTID=$(cat /opt/janabitech/core/keys/reality.sid)
fi



cat <<EOF > /opt/janabitech/xray/config.json
{
  "log": {
    "loglevel": "warning",
...
cat <<EOF > /tmp/janabitech-xray.service.tmp
[Unit]
Description=janabitech Xray-Core Engine
After=network.target

[Service]
Type=simple
User=root
WorkingDirectory=/opt/janabitech/xray
ExecStart=/opt/janabitech/xray/xray run -c /opt/janabitech/xray/config.json
Restart=always
RestartSec=3
LimitNOFILE=1048576
...
safe_deploy_systemd "janabitech-xray"
