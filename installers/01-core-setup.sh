...
if [ ! -f "$CONF_FILE" ]; then
    ...
    if [ -n "$IMAGITECH_DOMAIN" ]; then
        DOMAIN="$IMAGITECH_DOMAIN"
        NS_DOMAIN="$IMAGITECH_NS_DOMAIN"
        log_event "INFO" "Auto-configuring licensed domain: $DOMAIN"
    else
        read -p "Primary VPN Domain (e.g., vpn.imagitech.online): " DOMAIN < /dev/tty
        read -p "Nameserver Domain (e.g., ns-vpn.imagitech.online): " NS_DOMAIN < /dev/tty
    fi
