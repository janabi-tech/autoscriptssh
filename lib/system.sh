...
renew_ssl_cert() {
    ...
    systemctl stop janabitech-ws stunnel4 nginx apache2 >/dev/null 2>&1
    ...
    if [ -s /opt/janabitech/core/keys/fullchain.cer ]; then
        cat /opt/janabitech/core/keys/fullchain.cer /opt/janabitech/core/keys/private.key > /opt/janabitech/core/keys/stunnel.pem
        chmod 600 /opt/janabitech/core/keys/stunnel.pem
    ...
    # 5. Restore Services
    systemctl start janabitech-ws stunnel4
...
uninstall_script() {
    ...
    local services=(janabitech-ws janabitech-dnstt janabitech-monitor janabitech-udp-custom janabitech-xray stunnel4 dropbear danted)
    ...
    # Remove xray cron entry if present
    crontab -l 2>/dev/null | grep -v "xray purge-expired" | crontab -
...
update_script() {
    ...
    safe_fetch "lib/xray.sh" "/opt/janabitech/lib/xray.sh"
    ...
    safe_fetch "services/routing/async-ws-proxy.py" "/opt/janabitech/services/routing/ws-proxy.py"
    ...
    # Xray tables (idempotent)
    sqlite3 "$DB_PATH" "CREATE TABLE IF NOT EXISTS xray_users (id INTEGER PRIMARY KEY AUTOINCREMENT, username TEXT UNIQUE NOT NULL, protocol TEXT NOT NULL, uuid TEXT NOT NULL, expiry_date TEXT NOT NULL, max_logins INTEGER DEFAULT 0, data_limit BIGINT DEFAULT 0, data_usage BIGINT DEFAULT 0, status TEXT DEFAULT 'ACTIVE', created_at DATETIME DEFAULT CURRENT_TIMESTAMP);"
    sqlite3 "$DB_PATH" "CREATE TABLE IF NOT EXISTS xray_meta (k TEXT PRIMARY KEY, v TEXT);"
    ...
    systemctl daemon-reload
    systemctl restart janabitech-ws janabitech-monitor janabitech-xray >/dev/null 2>&1
...
create_backup() {
    ...
    tar -czf "$backup_file" core/database.db core/janabitech.conf core/keys/ core/vpn_passwd.bak core/vpn_shadow.bak >/dev/null 2>&1
...
restore_backup() {
    ...
    systemctl restart janabitech-ws janabitech-dnstt stunnel4 dropbear >/dev/null 2>&1
